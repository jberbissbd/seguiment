    ...
    def export_students(self, student_ids: list[int], destination: str | Path,
        ...
        for student_id in student_ids:
            ...
            try:
                ...
                if student is None:
                    raise EntityNotFoundError("L’alumne seleccionat no existeix.")
                base_name = self._safe_name(student.filing_name, "Alumne")
                key = base_name.casefold()
                used_folders[key] = used_folders.get(key, 0) + 1
                folder_name = (base_name if used_folders[key] == 1
                               else f"{base_name} ({used_folders[key]})")
                ...
                if include_documents and student is not None and student_root is not None:
                    try:
        ...
        return BatchExportResult(str(batch_root), exported, tuple(failures))
